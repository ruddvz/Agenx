...
## Phase 0B: Lifecycle Gate (Run After Phase 0, Before Any Task)

After reading CHANGELOG, ACTIVE, and EXECUTION-PLAN:

1. **Read `planning/PRODUCT-PHASES.md`** — find the `## Current Phase` block
2. **Classify the requested task** — which of the 12 lifecycle phases does it belong to?
3. **Apply the gate rule:**
   - Task phase == current phase → proceed
   - Task phase < current phase → check if decision was already written; proceed with caution
   - Task phase > current phase → DO NOT execute; log it; ask human to confirm
4. **Full gate logic is in `skills/project/lifecycle.md`** — load it for any ambiguous classification

**Gate enforcement is not optional.** A task dispatched out of phase sequence creates
downstream rework. The 60 seconds it takes to classify a task saves hours of fixing
mismatched assumptions between phases.
